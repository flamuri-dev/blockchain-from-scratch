...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        ...
        for current_block in chain {
            if hash(previous) != current_block.parent {
                return false;
            }
            if previous.height + 1 != current_block.height {
                return false;
            }
            if previous.state + current_block.extrinsic != current_block.state {
                return false;
            }
...
fn build_valid_chain(n: u64) -> Vec<Header> {
    ...
    for _ in 0..n {
        let block = chain[chain.len() - 1].child(0);
        chain.push(block);
    }
...
fn build_an_invalid_chain() -> Vec<Header> {
    ...
    let second_block = genesis_block.child(1);
    let thirt_block = second_block.child(2);
    let fourth_block = genesis_block.child(3); // This creates a fork

    vec![genesis_block, second_block, thirt_block, fourth_block]
}
...
fn build_forked_chain() -> (Vec<Header>, Vec<Header>) {
    ...
    let first_block = genesis_block.child(1);
    let second_block = first_block.child(2);
    
    let third_block = second_block.child(3);
    let fourth_block = third_block.child(4);
    let third_forked_block = second_block.child(5);
    let fourth_forked_block = third_forked_block.child(6);

    (
        vec![genesis_block.clone(), first_block.clone(), second_block.clone(), third_block, fourth_block],
        vec![genesis_block, first_block, second_block, third_forked_block, fourth_forked_block]
    )
